...
#if 1
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_encoder_open: h->param.rc.i_rc_method = %d \n", h->param.rc.i_rc_method );
        fflush(fp);
        fclose(fp);
    }
#endif
...
#ifndef GXH
    //h->param.ref_idx = param->ref_idx;
    COPY( ref_idx );
#if 0
    if( param->rc.i_rc_method == X264_RC_CQP )
    {
        COPY( rc.i_rc_method );
        COPY( rc.i_qp_constant );
        //h->param.rc.i_qp_min, h->param.rc.i_qp_max
        //h->param.rc.i_qp_constant = param->rc.i_qp_constant;
        if( x264_ratecontrol_new( h ) < 0 )
            printf("x264_encoder_try_reconfig: error");
    }
#endif
#if 0
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_encoder_try_reconfig: param->rc.i_rc_method= %d \n", param->rc.i_rc_method);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: param->rc.i_qp_constant= %d \n", param->rc.i_qp_constant);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: h->param.rc.i_qp_constant= %d \n", h->param.rc.i_qp_constant);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: h->param.rc.i_qp_min= %d \n", h->param.rc.i_qp_min);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: h->param.rc.i_qp_max= %d \n", h->param.rc.i_qp_max);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: param->rc.i_qp_min= %d \n", param->rc.i_qp_min);
        fprintf(fp,"gxh: x264_encoder_try_reconfig: param->rc.i_qp_max= %d \n", param->rc.i_qp_max);
        fflush(fp);
        fclose(fp);
#endif
#endif
...
#define BS_BAK_ROW_VBV        3
...
#if 1
    {
        ...
        fprintf(fp,"gxh: x264_slices_write: 0: h->sh.i_qp = %d \n", h->sh.i_qp );
        fprintf(fp,"gxh: x264_slices_write: 0: h->pps->i_pic_init_qp = %d \n", h->pps->i_pic_init_qp );
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_slices_write: x264_slice_header_write: h->sh.i_qp_delta = %d \n", h->sh.i_qp_delta );
...
cont:
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_slices_write: 2: h->sh.i_last_mb = %d \n", h->sh.i_last_mb );
...
static void *x264_slices_write( x264_t *h )
{
...
#if 1
    {
        ...
        fprintf(fp,"gxh: x264_slices_write: last_thread_mb = %d \n", last_thread_mb );
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if( h->param.rc.i_rc_method == X264_RC_CQP )
    {
        //i_global_qp = h->param.rc.i_qp_constant;
        //h->param.rc.i_qp_min, h->param.rc.i_qp_max
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_encoder_encode: i_global_qp = %d \n", i_global_qp );
        //fprintf(fp,"gxh: x264_encoder_encode: h->rc->qpm = %d \n", h->rc->qpm );
        fflush(fp);
        fclose(fp);
    }
#endif
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_encoder_encode: start slice: i_global_qp = %d \n", i_global_qp );
    ...
    {
        FILE *fp = fopen("x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: x264_encoder_encode: end slice: i_global_qp = %d \n", i_global_qp );
