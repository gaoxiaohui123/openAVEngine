...
SO_SNDBUF = 1024 * 1024
SO_RCVBUF = 1024 * 1024
#注意：音频与视频不同，视频根据发包速度，可以快进到当前时刻，音频的播放速度严格受声卡限制
# （这也是需要做音视频同步的原因）
LongMaxBufLen = 100
ShortMaxBufLen = 100
                                            ...
                                            for sockId2, [chanId2, base_time2] in targetPairs.items():
                                                ...
                                                if flag:
                                                    ...
                                                    if recv_time >= base_time2:
                                                        try:
                                                            sendDataLen = self.server.sock.sendto(data,
                                                                                                  (remoteHost,
                                                                                                   remotePort))
...
class SendTaskManagerThread(threading.Thread):
    ...
    def PushQueue(self, task_data):
        ...
        ret = self.server.load.lib.api_get_audio_extern_info(this_data, self.outparam)
...
class EchoServerThread(threading.Thread):
    ...
    def save_info(self, data, remoteHost, remotePort):
        self.outparam[0] = c_char_p(self.infobuf.raw)
        ret = self.load.lib.api_get_audio_extern_info(data, self.outparam)
        if ret > 0:
            outjson = str2json(self.outparam[0])
            if outjson != None:
                seqnum = outjson["seqnum"]
