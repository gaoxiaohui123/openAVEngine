...
class DecodeFrameThread(threading.Thread):
    ...
    def DecodeFrame(self, item):
        (data4, pktSize, complete, frame_timestamp, delaytime) = item
        #print("DecodeFrame: complete= ", complete)
        frame_info = complete.split(b",")
        ref_idc = int(frame_info[0]) - 1
        if delaytime > 500 and base.FRAMESKIP:
            if ref_idc in [0, 1]:
                pass
            else:
                self.idr_status = ref_idc + 1
                print("DecodeFrame: delaytime= ", delaytime)
                return
        #if ref_idc == 0:
        #    self.ref_idc_list = []
        ...
        if runflag and ret > 0:
            ...
            self.decode.param.update({"rtpSize": rtpSize})
            self.decode.param.update({"loss_rate": self.client.loss_rate})
...
class RecvTaskManagerThread(threading.Thread):
    def __init__(self, client):
        ...
        self.client = client
        self.decode = self.client.decode0
        self.lib = self.decode.load.lib
        #self.resortapi = self.lib.api_resort_packet
        self.id = self.client.id
        self.lock = threading.Lock()
        self.frame_decode = DecodeFrameThread(client)
        #(self.param, self.outbuf, self.outparam) = self.decode.setparam2()
        (self.param, self.outbuf, self.outparam) = self.decode.setparam3(loadlib.MAX_RESORT_FRAME_NUM)
        #(self.param, self.outbuf, self.outparam) = self.decode.setparam3(3)
    ...
    def Testtime(self, a1, a2, a3, a4, a5):
        self.lib.api_resort_packet(a1, a2, a3, a4, a5)
        return 0
    def ResortPacket(self, revcData, remoteHost, remotePort, recv_time, delaytime):
        ...
        ret = 0
        testtime0 = time.time()
        ret = self.lib.api_resort_packet(self.decode.handle, revcData, param_str, self.outbuf, self.outparam)
        #ret = self.Testtime(self.decode.handle, revcData, param_str, self.outbuf, self.outparam)
        testtime1 = time.time()
        #print("ResortPacket: 1")
        testdiff = int(1000 * (testtime1 - testtime0))
        if (testdiff > 0) and False:
        ...
        if ret > 0 and True:
            ...
            if self.frame_decode != None and pktSize != None:
                self.frame_decode.PushQueue((data4, pktSize, complete, frame_timestamp, delaytime))
                #self.ctime[0] = "0123456789012345"
    ...
    def ResortPacketAll(self, datalist, delaytime):
        self.param.update({"resort_type": 1})
        self.param.update({"min_distance": self.decode.min_distance})
        self.param.update({"delay_time": self.decode.delay_time})
        self.param.update({"delaytime": delaytime})
        self.param.update({"buf_size": self.decode.buf_size})
        self.param.update({"qos_level": QOS_LEVEL})
        self.param.update({"adapt_cpu": 0})
        self.param.update({"selfChanId": (self.client.id + 1)})
        ...
        for data in datalist:
            ...
            sizelist.append(len(revcData))
            ptrlist.append(revcData)
        size_arr = (c_int * n)(*sizelist)
        ptr_arr = (c_char_p * n)(*ptrlist)
        ##
        #print("ResortPacketAll: start")
        ret = self.lib.api_resort_packet_all(self.decode.handle, ptr_arr, size_arr, n, param_str, self.outbuf, self.outparam)
        #print("ResortPacketAll name= ", ret.contents.name)

        # print("ResortPacket n= ", n)
        if base.NETINFO:
            n = len(self.outparam[1].decode())
            #print("ResortPacketAll: self.outparam[1]= ", self.outparam[1])
            if n > 0:
                netDict = str2json(self.outparam[1])
                if netDict != None:
                    # print("ResortPacket: netDict= ", netDict)
                    ...
                    print("error: ResortPacketAll: netDict= ", netDict)
                    print("error: ResortPacketAll: self.outparam[1]= ", self.outparam[1])
                    pass
        loss_rate = ret.contents.loss_rate
        if loss_rate > 0:
            self.loss_rate = loss_rate
            self.client.loss_rate = loss_rate
            if loss_rate > 1:
                print(gTag + "TaskManagerThread: loss_rate= ", (loss_rate - 1))
        frame_num = ret.contents.frame_num
        if frame_num > 0 and True:
            #print("ResortPacketAll: frame_num= ", frame_num)
            if frame_num > loadlib.MAX_RESORT_FRAME_NUM:
                frame_num = loadlib.MAX_RESORT_FRAME_NUM
            offset = 0
            k = 0
            for i in range(frame_num):
                frame_timestamp = ret.contents.frame_timestamp[i]
                frame_size = ret.contents.frame_size[i]
                complete = ret.contents.complete[i]
                #print("ResortPacketAll: complete= ", complete)
                #print("ResortPacketAll: frame_size= ", frame_size)
                data4 = self.outbuf.raw[offset:(offset + frame_size)]
                offset += frame_size
                pkt_num = ret.contents.pkt_num[i]
                pktSize = []
                for j in range(pkt_num):
                    size = ret.contents.rtp_size[k]
                    #print("ResortPacketAll: size= ", size)
                    pktSize.append(size)
                    k += 1
                #print("ResortPacketAll: k= ", k)
                if self.frame_decode != None and pktSize != None:
                    self.frame_decode.PushQueue((data4, pktSize, complete, frame_timestamp, delaytime))
        #print(ret.contents.arr[0])
        #print(ret.contents.arr[1])
        #print(ret.contents.arr[2])
        testtime1 = time.time()
        testdiff = int(1000 * (testtime1 - testtime0))
        if (testdiff > 0) and False:
            print("ResortPacketAll testdiff= ", testdiff)
        return ret
    ...
    def run(self):
        ...
        while self.__running.isSet():
            ...
            if len(datalist) > 0:
                if base.RESORTTYPE: #delaytime > 500:
                    self.ResortPacketAll(datalist, delaytime)
                    #print("TaskManagerThread: len(datalist)= ", len(datalist))
                else:
                    for data in datalist:
                        # if data != None:
                        ...
                        loss_rate = loadlib.gload.lib.api_count_loss_rate2(self.decode.handle, revcData, len(revcData),
                                                                           90)
                        # continue  # test
...
class DataManager(object):
    ...
    def PopQueue(self):
        ...
        self.lock.release()
        return (ret, difftime)
    def ReducePkt(self, datalist):
        ret = []
        for data in datalist:
            (recvData, remoteHost, remotePort, recvTime) = data
            ref_idc = loadlib.gload.lib.api_get_ref_idc(recvData, 0, 0)
            if ref_idc in [0, 1]:
                ret.append(data)
        ret = datalist
        return ret
...
class EncoderClient(base.EchoClientThread):
    def __init__(self, params): #id, sessionId, actor, selfmode, modeId, width, height, host, port
    ...
    def init2(self):
        print("EncoderClient: init2")
        self.log_fp = None
        self.show = None
        self.capture = None
        try:
            filename = "../../mytest/enc_log_" + str(self.id) + ".txt"
        ...
        if chanId != None and info_status != None:
            ...
            if info_status == 1:
                if chanId == (self.id + 1):
                    ...
                    #print("video: push_net_info: chanId=", chanId)
                    pass
            else:
    ...
    def pop_net_info(self):
        ...
        for (netInfo, recvId) in self.otherNetInfoList:
            ...
            #ssrc = netInfo.get("ssrc")
            #info_status = netInfo.get("info_status")
            #item = (recvId, chanId, decodeId, ssrc, info_status)
            item = (recvId, chanId, decodeId)
            #print("pop_net_info: item=", item)
            #print("pop_net_info: netInfo=", netInfo)
            if item not in idlist:
                idlist.append(item)
                ret.append(netInfo)
                #print("pop_net_info: idlist=", idlist)
        #print("pop_net_info: ret=", ret)
        self.otherNetInfoList = []
        #if n > 0:
    ...
    def pop_self_net_info(self):
        ...
        for (netInfo, recvId) in self.selfNetInfoList:
            ...
            decodeId = netInfo.get("decodeId")
            #info_status = netInfo.get("info_status")
            #item = (recvId, chanId, info_status)
            item = (recvId, chanId, decodeId)
            #print("pop_self_net_info: item=", item)
            if item not in idlist:
                idlist.append(item)
                ret.append(netInfo)
                #print("pop_self_net_info: idlist=", idlist)
        self.selfNetInfoList = []
    ...
    def send_data(self, data, rtpSize):
        ...
        for csize in rtpSize:
            size = int(csize)
            data2 = data[sum:(sum + size)]
            #print("send_data: len(data2)= ", len(data2))
    ...
    def adapt_bitrate(self):
        ...
        if last_loss_rate >= 0:
            if last_loss_rate > 10:
                loss_rate = last_loss_rate / 100.0
                alfa = 0.8  # 0.5
                if loss_rate > 0.7:
                    alfa = 0.5
                elif loss_rate > 0.6:
                ...
                if self.encode0.bit_rate > self.start_bit_rate / 2:
                    #只降码率
                    #self.encode0.bit_rate = int(self.encode0.bit_rate * code_rate)
                    self.encode0.bit_rate = int(self.start_bit_rate * code_rate)
                    self.encode0.param.update({"bit_rate": int(self.encode0.bit_rate)})
                else:
                    #降码率,并skip帧,(进一步降码率的同时,降分辨率)
                    self.encode0.bit_rate = int(self.start_bit_rate / 2)
                    self.encode0.param.update({"bit_rate": int(self.encode0.bit_rate)})
                    ret = True
                    pass
                    # reduce resolution
                ...
                if last_loss_rate < 5 and self.encode0.bit_rate < self.start_bit_rate:
                    self.encode0.bit_rate = self.encode0.bit_rate + ((self.start_bit_rate - self.encode0.bit_rate) >> 1)
                    self.encode0.param.update({"bit_rate": int(self.encode0.bit_rate)})
        return ret
    def run(self):
        print("EncoderClient: run")
        i = 0
        self.maxLossRate = 0
            ...
            if len(data) > 0:
                ...
                if False:#ref_idx == 0:
                    ...
                    if last_loss_rate >= 0:
                        if last_loss_rate > 10:
                            ...
                            last_loss_rate = self.lib.api_get_loss_rate(self.encode0.handle, 2)
                            if last_loss_rate < 5 and self.encode0.bit_rate < self.start_bit_rate:
                                self.encode0.bit_rate = self.encode0.bit_rate + (
                                            self.start_bit_rate - self.encode0.bit_rate)
                                self.encode0.param.update({"bit_rate": int(self.encode0.bit_rate)})
                ###
                #if self.cmd_master != None:
                ...
                if len(netInfoList) > 0 and True:
                    ...
                    if self.maxLossRate > 0 and True:
                        ...
                        code_rate = (1.0 - loss_rate) #(1.0 - self.encode0.loss_rate)
                        code_rate = code_rate * alfa
                        code_rate = float(int(code_rate * 100)) / 100.0
                        if code_rate < 0.15:
                            code_rate = 0.15
...
class DecoderClient(base.EchoClientThread):
    def __init__(self, params): #id, sessionId, actor, width, height, host, port
        base.EchoClientThread.__init__(self, params)
        self.id = self.chanId
        self.iNetTime = int(self.nettime * 1000)
        self.stream = None
    ...
    def init2(self):
        ...
        self.log_fp = None
        try:
            filename = "../../mytest/dec_log_" + str(self.id) + ".txt"
            self.log_fp = open(filename, "w")
        except:
        ...
        self.recv_task = None #RecvTaskManagerThread(self)
        # self.recv_task.start()
    def __del__(self):
    ...
    def push_net_info(self, netInfoList):
            ...
            if info_status == 1:
                ...
                for chanId in self.encChanIdList:
                    ...
                    if chanId not in self.hasNetList:
                        ...
                        continue
                    ###
                    #print("DecoderClient: push_net_info: type(self.selfmode)= ", type(self.selfmode))
                    #print("DecoderClient: push_net_info: (self.selfmode > 0)= ", (self.selfmode > 0))
                    #flag = (chanId0 != (chanId + 1)) or (self.selfmode > 0)
                    #print("DecoderClient: push_net_info: flag= ", flag)
        ...
        #            thisClient.push_net_info(thisInfo, self.id)
            ###
            #chanId = thisInfo.get("chanId")
            #if chanId != None:
...
def RunSessionStart(base_idx, proc_idx, speakernum, host, port):
    ...
    if True:
        ...
        rectList = []
        rectList.append([0, 0, SHOW_WIDTH, SHOW_HEIGHT])
        rectList.append([0, 0, SHOW_WIDTH >> 2, SHOW_HEIGHT >> 2])
        #rectList.append([0, 0, SHOW_WIDTH, SHOW_HEIGHT])
        thread_show.SetRects(rectList)
        ...
        for i in range(base_idx, speakernum, 1):
            ...
            for j in range(speakernum):
                ...
                (params.modeId, params.avtype, params.selfmode) = (base.DEFAULT_MODE, 0, 0)
                ...
                if actor in [1, 2]:
                    ...
                    threadlist[k].yuvfilename = '/home/gxh/works/datashare/FourPeople_1280x720_30.yuv'
                    devicetype = -1
                    threadlist[k].framerate = 25
                    threadlist[k].encode0.refs = SVC_REFS
                    threadlist[k].fec_level = 2  # 1#0#2#0#1#2#3#0#4 #3 #2#0
                    threadlist[k].encode0.enable_fec = 1
                    threadlist[k].encode0.loss_rate = 0.1  # 0.2 #0.3
                    threadlist[k].encode0.code_rate = (1 - threadlist[k].encode0.loss_rate)
                    bitrate = int(threadlist[k].encode0.code_rate * bitrate)
                    threadlist[k].encode0.bit_rate = bitrate
                    threadlist[k].encode0.mtu_size = mtu_size
                    threadlist[k].encode0.setparam()
                    threadlist[k].opendevice(devicetype)
                    ...
                    threadlist[k].decode0.buf_size = (1 << buffer_shift)  # 1024#必须是2的指数
                    threadlist[k].decode0.mtu_size = mtu_size
