...
class SendThread(threading.Thread):
    ...
    def getinfo(self, now_time, sendDataLen):
        ...
        if difftime > self.interval and self.start_time > 0:
            ...
            # bitrate /= (1024 * 1024)
            if bitrate > 0:
                sessionId = (self.idx >> 16)
                chanId = (self.idx & 0xFFFF)
                if bitrate > 1024:
    ...
    def getinfo(self, now_time, revDataLen):
        ...
        if difftime > 2000 and self.start_time > 0:
            ...
            if bitrate > 0:
                ...
                if bitrate > 1024:
                    bitrate = (bitrate + 512) >> 10
                    print(gTag + "SendThread: run: recvfrom: bitrate(Mbps)= ", (sessionId, chanId, bitrate))
                else:
                    print(gTag + "SendThread: run: recvfrom: bitrate(Kbps)= ", (sessionId, chanId, bitrate))
            self.start_time = now_time
...
class SocketProcessThread(threading.Thread):
    ...
    def CmdProcess(self, data):
            ...
            if jsondata != None:
                ...
                if sockItem == None:
                    ...
                    if True:
                        ...
                        sockThread.start()
                    self.socketDict.update({sockId: (sockThread, sessionId, chanId)})
                else:
                    (sockThread, sessionId2, chanId2) = sockItem
                if True:
                    #会影响转发率
                    sockId = remoteHost + "_" + str(remotePort)
        ...
        return iscmd
    def RawProcess(self, data):
        (revcData, remoteHost, remotePort, recv_time) = data
        sockId = remoteHost + "_" + str(remotePort)
        sockItem = self.socketDict.get(sockId)
        if sockItem != None:
            (sockThread, sessionId, chanId) = sockItem
            sessionInfo = self.sessionDict.get(sessionId)
            if sessionInfo != None:
                item = revcData.decode().split(",")
                seqnum = int(item[0])
                ...
                for thisactor in actors:
                    ...
                    if decodeActors != None:
                        for (sockId2, chanId2, recv_time2) in decodeActors:
                            ...
                            if sockItem2 != None:
                                (sockThread2, sessionId2, chanId2) = sockItem2
                                if sockThread2 != None and sessionId2 == sessionId and chanId2 == chanId:
