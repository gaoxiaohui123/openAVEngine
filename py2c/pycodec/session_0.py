...
class AVSession(threading.Thread):
    def __init__(self, windId, params, width, height, isCreater):
    ...
    def create_up_video(self, chanId, devicePos, deviceDict):
        ...
        (id, sessionId, actor, selfmode) = (chanId, 100, 1, 0)
        ...
        video_thread = videoenc(id, sessionId, actor, selfmode, self.modeId, self.width, self.height, host, port)
        video_thread.init()
    ...
    def create_up_audio(self, chanId, devicePos, deviceDict):
        ...
        port = port + 1
        audio_thread = audioenc(id, sessionId, actor, selfmode, self.modeId, host, port)
    ...
    def create_down_video(self, chanId, devicePos):
        (id, sessionId, actor) = (chanId, 100, 3)
        conf = self.conferenceInfo
        sessionId = conf["sessionId"]
        sup = self.general["supprams"]
        spatiallayer = int(sup["spatiallayer"]["name"])
        ...
        (bitrate, mtu_size, fec_level, buffer_shift) = self.GetParams(refs, self.width, self.height)
        video_thread = vidodec(id, sessionId, actor, self.width, self.height, host, port)
        video_thread.init()
        self.AddSpeakerId(video_thread)
        ###video
