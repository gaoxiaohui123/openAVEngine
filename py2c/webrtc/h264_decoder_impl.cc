...
int32_t H264DecoderImpl::InitDecode(const VideoCodec* codec_settings,
  ...
  if(_testfile == nullptr){
    _testfile = fopen("/home/yang/decode_gxh.yuv","wb");
  }
...
int32_t H264DecoderImpl::Decode(const EncodedImage& input_image,
  ...
  if(fp == nullptr){
    //fp = fopen("/home/yang/recive.h264","wb");
  }
  if(fp !=nullptr){
      fwrite(input_image.data(),input_image.size(),1,fp);
      fflush(fp);
  }
  ...
  if (av_frame_->width != i420_buffer->width() ||
      ...
      printf("Decode: av_frame_->width= %d  \n", av_frame_->width);
      printf("Decode: av_frame_->height= %d  \n", av_frame_->height);
      printf("Decode: i420_buffer->width()= %d  \n", i420_buffer->width());
      printf("Decode: i420_buffer->height()= %d  \n", i420_buffer->height());
    ...
    if(_testfile)
    {
      rtc::scoped_refptr<const I420BufferInterface> frame_buffer =cropped_frame.video_frame_buffer()->ToI420();
      int w = frame_buffer->width();
      int h = frame_buffer->height();
      int stride = i420_buffer->width();
      ...
      for (int i = 0; i < h; i++)
      {
        fwrite(&src_y[i * stride], w, 1, _testfile);
        //memmove(&dst_y[i * w], &src_y[i * stride], w);
      ...
      for (int i = 0; i < h; i++)
      {
        if (!(i & 1))
        {
            fwrite(&src_u[(i >> 1) * (stride >> 1)], w >> 1, 1, _testfile);
            //memmove(&dst_u[(i >> 1) * (w >> 1)], &src_u[(i >> 1) * (stride >> 1)], w >> 1);
        }
      }
      //V
      for (int i = 0; i < h; i++)
      {
        if (!(i & 1))
        {
            fwrite(&src_v[(i >> 1) * (stride >> 1)], w >> 1, 1, _testfile);
            //memmove(&dst_v[(i >> 1) * (w >> 1)], &src_v[(i >> 1) * (stride >> 1)], w >> 1);
        }
      }
#else
      fwrite(src_y,w * h,1,_testfile);
      fwrite(src_u,(w >> 1) * (h >> 1),1,_testfile);
      fwrite(src_v,(w >> 1) * (h >> 1),1,_testfile);
#endif
      fflush(fp);
    ...
    if(_testfile)
    {
      rtc::scoped_refptr<const I420BufferInterface> frame_buffer =decoded_frame.video_frame_buffer()->ToI420();
      int w = frame_buffer->width();
      int h = frame_buffer->height();
      uint8_t *src_y = const_cast<uint8_t*>(frame_buffer->DataY());
      uint8_t *src_u = const_cast<uint8_t*>(frame_buffer->DataU());
