                       ...
                       const x264_nal_t *nals, int nnal)
{
    ...
    for (i = 0; i < nnal; i++)
    {
        ...
        ctx->nal_size[i] = nals[i].i_payload;
    }
...
static void reconfig_encoder(AVCodecContext *ctx, const AVFrame *frame)
{
...
#ifndef GXH
  if (ctx->ref_idx > 0) //x4->ref_idx >= 0 ||
  {

    x4->params.ref_idx = ctx->ref_idx;
    x264_encoder_reconfig(x4->enc, &x4->params);
  }
  if(ctx->qmax == ctx->qmin)
  {
    x4->params.rc.i_rc_method = X264_RC_CQP;
    x4->params.rc.i_qp_constant = ctx->qmax;
    x4->params.rc.i_qp_min = ctx->qmin;
    x4->params.rc.i_qp_max = ctx->qmax;
    x264_encoder_reconfig(x4->enc, &x4->params);
  }
  else //if(ctx->bit_rate <= 0)
  {
    ...
    //x4->params.rc.i_rc_method = X264_RC_CQP;
    //x4->params.rc.i_qp_constant = ctx->qmax;
    //x4->params.rc.i_qp_step = 10;
    //x4->params.rc.i_qp_min = x4->params.rc.i_qp_constant;
    //x4->params.rc.i_qp_max = x4->params.rc.i_qp_constant;
    x264_encoder_reconfig(x4->enc, &x4->params);
  }
#if 0
    FILE *fp = fopen("ff_x264_gxh.txt", "ab+");
    fprintf(fp,"gxh: reconfig_encoder: ctx->ref_idx= %d \n", ctx->ref_idx);
    fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_rc_method= %d \n", x4->params.rc.i_rc_method);
    fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_qp_constant= %d \n", x4->params.rc.i_qp_constant);
    fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_qp_min= %d \n", x4->params.rc.i_qp_min);
    fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_qp_max= %d \n", x4->params.rc.i_qp_max);
    fflush(fp);
    fclose(fp);
#endif
  ...
  if (x4->avcintra_class < 0) {
    ...
    if (x4->params.rc.i_rc_method == X264_RC_ABR &&
        ...
        //reset biterate @ note add by gxh
        x4->params.rc.i_bitrate = ctx->bit_rate / 1000;
        x264_encoder_reconfig(x4->enc, &x4->params);
    }
                      ...
                      int *got_packet)
{
...
#ifndef GXH
    //refer to: parse_qpfile
    //if(ctx->bit_rate <= 0)
    {
        x4->pic.i_qpplus1 = x4->params.rc.i_qp_constant + 1;
    }
#if 0
    {
        FILE *fp = fopen("ff_x264_gxh.txt", "ab+");
        ...
        fprintf(fp,"gxh: X264_frame: start: x4->pic.i_qpplus1= %d \n", x4->pic.i_qpplus1);
        fprintf(fp,"gxh: X264_frame: start: ctx->bit_rate= %d \n", ctx->bit_rate);
        //fprintf(fp,"gxh: X264_frame: start: ctx->ref_idx= %d \n", ctx->ref_idx);
        fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_qp_min= %d \n", x4->params.rc.i_qp_min);
        fprintf(fp,"gxh: reconfig_encoder: x4->params.rc.i_qp_max= %d \n", x4->params.rc.i_qp_max);
    ...
    {
        FILE *fp = fopen("ff_x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: X264_frame: end: ctx->ref_idx= %d \n", ctx->ref_idx);
        fflush(fp);
        fclose(fp);
    }
...
static av_cold int X264_init(AVCodecContext *avctx)
{
    ...
    {
        FILE *fp = fopen("ff_x264_gxh.txt", "ab+");
        fprintf(fp,"gxh: X264_init: x4->ref_idx= %d \n", x4->ref_idx);
        fflush(fp);
        fclose(fp);
    }
#endif
...
FF_ENABLE_DEPRECATION_WARNINGS
...
#ifndef GXH
  if (avctx->ref_idx >= 0) //x4->ref_idx >= 0 ||
  {

    x4->params.ref_idx = avctx->ref_idx;
    //x264_encoder_reconfig(x4->enc, &x4->params);
  }
  if(avctx->qmax == avctx->qmin)
  {
    x4->params.rc.i_rc_method = X264_RC_CQP;
    x4->params.rc.i_qp_constant = avctx->qmax;
    x4->params.rc.i_qp_min = avctx->qmin;
    x4->params.rc.i_qp_max = avctx->qmax;
    //x264_encoder_reconfig(x4->enc, &x4->params);
  }
  ...
  {
    FILE *fp = fopen("ff_x264_gxh.txt", "ab+");
    //fprintf(fp,"gxh: X264_init: avctx->ref_idx= %d \n", avctx->ref_idx);
    fprintf(fp,"gxh: X264_init: x4->params.rc.i_qp_min= %d \n", x4->params.rc.i_qp_min);
    fprintf(fp,"gxh: X264_init: x4->params.rc.i_qp_max= %d \n", x4->params.rc.i_qp_max);
    fflush(fp);
    fclose(fp);
  }
#endif
...
static const AVOption options[] = {
    ...
    //{ "ref-idx","added by gxh for control ref idx",           OFFSET(ref_idx),AV_OPT_TYPE_INT,    { .i64 = -1 }, -1, INT_MAX, VE },
